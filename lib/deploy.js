...
function findAppName() {
    var data = fs.readFileSync('.project');
    var lines = data.toString().split("\n");
    var appName;

    for(i = 0; i < lines.length; i++) {
        if (lines[i].indexOf('<name>') >= 0) {
            appName = lines[i].substring(lines[i].indexOf('<name>') + 6, lines[i].indexOf('</name>'));
            break;
        }
    }

    if (appName === undefined) {
        console.log(clc.redBright("\nwas not possible to find your application name"));
        console.log(clc.redBright("pleas check your ") + clc.yellowBright(".project") + clc.redBright(" file inside the app's folder"));
        process.exit(1);
    }
...
function mongoRestore() {
    ...
    if (env.restore) {
        ...
        console.log(clc.blueBright("restoring ") + clc.yellowBright(restoreSubject) + clc.blueBright(" mongo db"));
        
        var command;
        var source = restoreSubject;
        ...
        if (restoreTo === 'local') {
            command = "cd ./.deploy && mongorestore -h 127.0.0.1 --port " + configFile.local.mongodb.port + " -d meteor dump/" +  source;
        }
        else {
            command = "cd ./.deploy && mongorestore -h " + configuration.mongodb.serverAddress + " --port " + configuration.mongodb.port + " -u " + configuration.mongodb.user + " -p " + configuration.mongodb.password + " -d " + configuration.mongodb.dbName + " dump/" + source;
        }
...
function startForever(ssh) {
    ...
    if (configuration.sshAddress === configuration.mongodb.serverAddress) dbAddress = 'localhost';
    else dbAddress = configuration.mongodb.serverAddress;

    //export mongodb variables
    var command;
    if (env.disableWS) {
        command = "cd " + configuration.webDirectory + " && export MONGO_URL='mongodb://" + configuration.mongodb.user + ":" + configuration.mongodb.password + "@" + dbAddress + ":" + configuration.mongodb.port + "/" + configuration.mongodb.dbName + "' && export ROOT_URL='http://" + configuration.vHost.domainName + "'" + " && export PORT=" + configuration.port + " && export DISABLE_WEBSOCKETS=true && forever start " + configuration.foreverProcessName;
        logDetail = clc.yellowBright("without web-sockets");
    }
    else {
        command = "cd " + configuration.webDirectory + " && export MONGO_URL='mongodb://" + configuration.mongodb.user + ":" + configuration.mongodb.password + "@" + dbAddress + ":" + configuration.mongodb.port + "/" + configuration.mongodb.dbName + "' && export ROOT_URL='http://" + configuration.vHost.domainName + "'" + " && export PORT=" + configuration.port + " && forever start " + configuration.foreverProcessName;
        logDetail = clc.greenBright("with web-sockets");
    }

    console.log(clc.blueBright("exporting variables and starting forever..."));
    console.log(clc.blueBright("application will be started ") + logDetail + "\n");
    ...
    if (env.restore) {
        ...
        if ((env.deploy) && (restoreTo !== undefined) && (deployTo !== undefined)) {
            if (restoreTo !== deployTo) {
                ...
                console.log(clc.yellowBright("the monto restore will be executed to ") + clc.greenBright(deployTo) + "\n");
            }
        }
...
if (readConf) {
    ...
    configuration.appnName = configFile[usedDeployPosition].appName;
