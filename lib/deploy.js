...
function createMongoDb() {
    exec("mongo -u " + configuration.mongodb.rootUser + " -p " + configuration.mongodb.rootPass + " --host " + configuration.mongodb.serverAddress + " --port " + configuration.mongodb.port + " --authenticationDatabase " + configuration.mongodb.rootAuthDb, function(error, stdout, stderr) {
        ...
        ssh.exec('cd /home/' + configuration.sshUser + ' && mkdir -p ' + configuration.appName, function(err, stream) {
            ...
            stream.on('exit', function(code, signal) {
                ...
                client.on('transfer', function(buffer, uploaded, total) {
                    if (total - uploaded < 2) uploaded = total;
